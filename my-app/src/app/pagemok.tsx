...
export default function Home() {
  ...
  const animationTargetRef = useRef<number | null>(null) // Track animation target
  ...
  const animateToTarget = useCallback((targetScroll: number, quick: boolean = false) => {
    ...
    animationTargetRef.current = targetScroll // Track the target
  ...
  // Ensure animation completes to a final state (0 or maxScroll)
  const ensureCompleteState = useCallback(() => {
    const container = chatContainerRef.current
    if (!container || isAnimatingRef.current) return
    
    const currentScroll = container.scrollTop
    // If we're in the animation zone but not at either end, snap to nearest
  ...
  // Snap to nearest edge - uses 50% threshold
  const snapToNearest = useCallback((quick: boolean = false) => {
    ...
    // Always use 50% threshold for snapping
    const targetScroll = progress >= 0.5 ? maxScroll : 0
    
    animateToTarget(targetScroll, quick)
  }, [animateToTarget])

  // Handle scroll - only update state, don't cancel animation
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    // If animation is running, skip - the animation updates scrollY directly
    ...
    // If user is interacting (touch/touchpad), update scroll position
    if (isUserInteractingRef.current) {
      setScrollY(currentScroll)
    } else {
      // Not animating and not user interacting - just update state
      setScrollY(currentScroll)
    }
  }
  ...
  const handleWheel = (e: React.WheelEvent<HTMLDivElement>) => {
    ...
    if (delta >= 50) {
      // Handle scrolling in the animation zone
      if (currentScroll >= 0 && currentScroll <= maxScroll) {
        const targetScroll = e.deltaY > 0 ? maxScroll : 0
        
        // Check if we're already animating to this target - if so, let it complete
        if (isAnimatingRef.current && animationTargetRef.current === targetScroll) {
          return
        }
        
        // Only animate if we're not already at the target
        const shouldAnimate = (targetScroll === maxScroll && currentScroll < maxScroll) ||
      ...
      // Safety check: ensure we reach a complete state after mouse wheel scrolling
    ...
    // Touchpad: small delta (< 50)
      ...
      // Cancel any ongoing animation on wheel input
      if (animationFrameRef.current) {
        cancelAnimationFrame(animationFrameRef.current)
        animationFrameRef.current = null
      }
      ...
      // Check if we're in or near the animation zone
      if (currentScroll >= 0 && currentScroll <= maxScroll) {
        ...
        wheelTimeoutRef.current = setTimeout(() => {
          ...
          if (container) {
            ...
            // Snap if we're in the middle of the animation zone
        ...
        // Outside animation zone, reset user interaction
        isUserInteractingRef.current = false
      }
    }
  }

  // Touch handlers - interrupt animation on touch
  ...
  const handleTouchMove = () => {
    // Keep user interacting flag true during move
    isUserInteractingRef.current = true
    // Cancel any ongoing animation
    if (animationFrameRef.current) {
      cancelAnimationFrame(animationFrameRef.current)
  ...
  const handleTouchEnd = () => {
    ...
    // Clear any wheel timeout that might interfere
    if (wheelTimeoutRef.current) {
      clearTimeout(wheelTimeoutRef.current)
      wheelTimeoutRef.current = null
    }
